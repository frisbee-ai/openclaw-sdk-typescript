...
import { EventManager, createEventManager, MAX_EVENT_NAME_LENGTH } from './event.js';
...
describe('EventManager', () => {
  ...
  describe('off', () => {
    ...
    it('should remove handler across all namespaces when namespace not specified', () => {
      const handler = vi.fn();

      events.on('test', handler, 'ns1');
      events.on('test', handler, 'ns2');
    ...
    it('should remove all handlers for pattern across all namespaces', () => {
      const ns1Handler = vi.fn();
      const ns2Handler = vi.fn();
    ...
    it('should remove only specific namespace when namespace specified', () => {
      const ns1Handler = vi.fn();
      const ns2Handler = vi.fn();
