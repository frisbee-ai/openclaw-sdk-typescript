...
describe("ReconnectManager", () => {
  ...
  describe("basic functionality", () => {
    ...
    it("should check if reconnecting", async () => {
      ...
      // Start reconnect process with very short timeout
      const quickManager = createReconnectManager({
        maxAttempts: 2,
        initialDelayMs: 10,
        maxDelayMs: 20,
      ...
      expect(quickManager.isReconnecting()).toBe(true);
  ...
  describe("abort", () => {
    it("should stop reconnection", async () => {
      ...
      // Start reconnection with high maxAttempts
      ...
      // The promise should resolve successfully (connectFn completed)
      await reconnectPromise;
  ...
  describe("retryable auth errors", () => {
    ...
    it("should stop after max auth retries", async () => {
      ...
      await expect(
        quickManager.reconnect(connectFn, refreshTokenFn),
      ).rejects.toThrow("Max auth retries");
  ...
  describe("token refresh failure", () => {
    it("should stop reconnecting when token refresh fails", async () => {
      ...
      await expect(
        manager.reconnect(connectFn, refreshTokenFn),
